...
and bench/bench_cuda_v4_num_splits.py).

Mirrors tests/test_kernel_v4.py's structure (split-invariance, a tight
num_splits=1 equivalence check, a named oversplit edge case) against
src/kernel_cuda_v4.py instead of the Triton split-K kernel — built on
CUDA v3's warp-shuffle reduction, so the equivalence check here is against
CUDA v3, not CUDA v1 (see cuda/kernel_v4_split_k.cu's module docstring).
"""
...
def _compare(q, k_cache, v_cache, block_table, seq_lens, num_splits=8):
    compare_to_reference(
        paged_attention_decode_cuda_v4,
...
def test_num_splits_one_matches_cuda_v3_exactly():
    # cuda/kernel_v4_split_k.cu: at num_splits=1, phase 1's loop covers the
    # whole sequence in one chunk (byte-for-byte CUDA v3's loop) and
    # phase 2's merge degenerates to a single term. Measured directly
    # before picking a tolerance: max diff 0.0 at this exact shape.
    ...
    v4_out = paged_attention_decode_cuda_v4(q16, k16, v16, bt_cuda, sl_cuda, num_splits=1)

    torch.testing.assert_close(v4_out, v3_out, rtol=1e-4, atol=1e-5)
