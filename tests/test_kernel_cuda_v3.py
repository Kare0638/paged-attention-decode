...
Mirrors tests/test_kernel_cuda_v1.py's shape matrix exactly, against
src/kernel_cuda_v3.py (the warp-shuffle-reduction kernel) instead of CUDA
v1 — same fp16-vs-fp32-reference tolerance. Plus a v1-vs-v3 comparison:
v3 computes the same score as v1 (same terms, same scale) but reduces
them in a different order (warp-shuffle tree vs. v1's shared-memory
tree), so floating-point addition's non-associativity means this is
expected to be *close*, not bit-exact like v2 was against v1 — the
tolerance below was set from the measured diff, not guessed.
"""
...
def _compare(q, k_cache, v_cache, block_table, seq_lens):
    compare_to_reference(paged_attention_decode_cuda_v3, q, k_cache, v_cache, block_table, seq_lens)
...
def test_matches_cuda_v1_closely():
    # v3 computes the same score terms as v1 but reduces them in a
    # different order (warp-shuffle tree vs. v1's shared-memory tree) —
    # expected close, not bit-exact (floating-point addition isn't
    # associative), unlike v2's bit-exact match against v1. Measured
    # directly at this exact shape before picking a tolerance: max diff
    # ~6e-8 (both reductions accumulate in fp32 throughout, only rounding
    # to fp16 once at the final store, so the reordering barely matters
    # in practice) — rtol/atol below has real headroom above that, not a
    # guess.
    ...
    k_cache, v_cache = make_cache(num_physical_pages, page_size, num_kv_heads, head_dim, seed=55)
    q = torch.randn(batch, num_q_heads, head_dim, dtype=torch.float32)
    ...
    v1_out = paged_attention_decode_cuda_v1(q16, k16, v16, bt_cuda, sl_cuda)
    v3_out = paged_attention_decode_cuda_v3(q16, k16, v16, bt_cuda, sl_cuda)

    torch.testing.assert_close(v3_out, v1_out, rtol=1e-4, atol=1e-5)
