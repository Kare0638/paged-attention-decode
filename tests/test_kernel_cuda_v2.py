...
src/kernel_cuda_v2.py (the batched-shared-memory-reduction kernel) instead
of CUDA v1 — same fp16-vs-fp32-reference tolerance. Plus one v2-specific
check: v2 is a pure reassociation of v1's score computation (same
floating-point operations, same order within each row, just batched
sync-wise across rows), so it should match CUDA v1 directly at a tight
tolerance — not just each independently against the looser fp32-reference
tolerance — the same evidentiary role as test_kernel_v4.py's
num_splits=1-vs-v1 bit-exact check in the Triton line.
"""
...
def _compare(q, k_cache, v_cache, block_table, seq_lens):
    compare_to_reference(paged_attention_decode_cuda_v2, q, k_cache, v_cache, block_table, seq_lens)
...
def test_matches_cuda_v1_closely():
    # v2 is a pure reassociation of v1's score computation (same
    # floating-point ops, same per-row order, just batched sync-wise
    # across rows) — should match near bit-for-bit, a much tighter bar
    # than the shared fp32-reference tolerance.
    ...
    v2_out = paged_attention_decode_cuda_v2(q16, k16, v16, bt_cuda, sl_cuda)

    torch.testing.assert_close(v2_out, v1_out, rtol=1e-4, atol=1e-5)
