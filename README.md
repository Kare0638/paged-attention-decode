...
- NCU profiling on A100: tried, not skipped — `ncu` is installed on the pod but every profiling attempt fails with `ERR_NVGPUCTRPERM`, confirmed to be a structural RunPod platform limitation (containers aren't privileged, no `CAP_SYS_ADMIN` option), not a configuration issue on this project's end. A re-tuned `num_splits` sweep for A100 remains undone by explicit scope choice. Full writeup, including a RunPod SSH-proxy connectivity gotcha worth knowing about, in [`profiles/notes.md`](profiles/notes.md#a100-cross-hardware-validation-2026-08-07).
