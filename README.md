...
Decode-phase attention is memory-bandwidth-bound, not compute-bound: for a Qwen2.5-1.5B-style config (GQA ratio 6, head_dim 128, fp16 KV cache), the arithmetic intensity works out to ~6 FLOP/byte, far below the ~36 FLOP/byte ridge point of an RTX 3060 — so almost every optimization here is about memory access patterns and occupancy, not raw FLOPs.
...
- [ ] Correctness: fp32 PyTorch reference implementation
...
- [ ] Correctness test suite (page-boundary, ragged batch, GQA ratios, fuzz)
