...
Decode-phase attention is memory-bandwidth-bound, not compute-bound: for a Qwen2.5-1.5B-style config (GQA ratio 6, head_dim 128, fp16 KV cache), the arithmetic intensity works out to ~6 FLOP/byte, far below this RTX 3060 Laptop's **measured** ridge point of ~84 FLOP/byte (26.4 TFLOPS fp16 matmul / 314 GB/s D2D — both measured on this machine, not nominal spec-sheet numbers; see [bench/results/](bench/results/)) — so almost every optimization here is about memory access patterns and occupancy, not raw FLOPs. See [`bench/roofline.py`](bench/roofline.py) / [profiles/roofline.png](profiles/roofline.png) for the checked-in derivation and a roofline plot of all 8 kernel versions across the batch sweep.
...
- NCU profiling on A100: tried, not skipped — `ncu` is installed on the pod but every profiling attempt fails with `ERR_NVGPUCTRPERM`, confirmed to be a structural RunPod platform limitation (containers aren't privileged, no `CAP_SYS_ADMIN` option), not a configuration issue on this project's end. Full writeup, including a RunPod SSH-proxy connectivity gotcha worth knowing about, in [`profiles/notes.md`](profiles/notes.md#a100-cross-hardware-validation-2026-08-07).
- **`num_splits` re-swept on A100** (same methodology, batch=1): the two kernels tell opposite stories. CUDA v4's `num_splits=64` default turns out to already be optimal on A100 too (20.63x vs. `cuda_v3`, essentially matching the laptop's 20.75x at the same value) — no retuning was actually needed. Triton v4's whole `num_splits` axis nearly flattens on A100 instead — every value from 1 to 64 lands within 1.51x-1.55x of `v1` (vs. the laptop's real 1.22x-1.88x spread), plausibly because A100 has enough SMs (~108 vs. the laptop's ~28) that split-K's occupancy fix has much less left to buy once the kernel is already this fast in absolute terms. Neither shipped default was changed — both remain reasonable on A100, confirmed rather than assumed. Full tables in [`profiles/notes.md`](profiles/notes.md#num_splits-re-swept-on-a100-2026-08-07).
- **FlashInfer on A100** (`bench/results/flashinfer_comparison_a100.json`): FlashInfer wins at every batch size, as on the laptop — but its edge over this project's own `v4` is *larger* on A100 (2.21x at batch=1, 1.85x at batch=16, 1.41x at batch=64, vs. the laptop's 2.07x/1.50x/1.16x), and the gap to `cuda_v4` keeps widening instead of plateauing (5.24x → 8.06x → 17.23x, vs. the laptop's 6.17x → 12.70x → 13.02x). Plausible reading: FlashInfer has kernel paths tuned specifically for Ampere datacenter cards, while this project's `num_splits` defaults were swept once on the laptop and never retuned for A100 — a real cost of not retuning per GPU, not a discrepancy to explain away. Full table in [`profiles/notes.md`](profiles/notes.md#flashinfer-comparison-on-a100-2026-08-07).
